...
foreach($API_NOTIFY_FILE in Get-ChildItem -Path "$($args[0])generated\csharp" -Filter "*Spi.cs") {
    foreach($line in Get-Content $API_NOTIFY_FILE.FullName) {
    ...
    foreach($line in Get-Content $API_NOTIFY_FILE.FullName) {
        if($line -match $CLASS_REG_EXP) {
            Add-Content $TARGET_FILE "public class $($API_NOTIFY_FILE.BaseName)EventCallback : $($matches[1])"
